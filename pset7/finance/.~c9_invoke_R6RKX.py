...
def buy():
    ...
    if request.method == "POST":
        ...
        if credit >= stock_cost:
            transaction = db.execute("INSERT INTO portfolio(stock, stock_name, price, shares, buy_date, user_id) VALUES(:stock, :name, :price, :shares, :date, :user_id)", stock = symbol, name = name, price = price, shares = shares, date=datetime.now(), user_id = session["user_id"])

            payment = db.execute("UPDATE users SET cash = cash - :stock_cost WHERE id = :user_id", stock_cost = stock_cost, user_id = session['user_id'])
...
def sell():
    ...
    if request.method == "POST":
        ...
        if shares <= 0:
            return apology("please provide positive number of shares", 402)"""

        shares_owned = db.execute("SELECT shares FROM portfolio WHERE user_id = :user_id AND stock = :stock", user_id=session["user_id"],stock = stock, )

        #shares_int = 0
        #for line in shares_owned:
        #    shares_int += int(line["shares"]

        if shares_owned < shares:
            return apology("you have not enough shares", 405)

        #else:
         #   shares_new = shares_int - shares1
          #  shares_db = db.execute("UPDATE portfolio SET shares = :shares_new WHERE stock = :stock AND user_id = :user_id", stock = request.form.get("symbol"), user_id=session["user_id"])
           # return redirect("/", 200) ## delete this
